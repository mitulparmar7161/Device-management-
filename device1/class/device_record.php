...
public function fetchData(){
    ...
    if(isset($_GET['sort'])){
      ...
      $query="select * from device_record  WHERE CONCAT(`request_time`,`assigned_time`, `return_time`,`reject_time`,`status`) LIKE '%".$valueToSearch."%'  order by $colunm $sort LIMIT $start_from, $limit";

    }

    else{

    $query="select user.name , user.id, device.id, device.name,device_record.device_id,device_record.request_time,device_record.assigned_time,device_record.return_time,device_record.reject_time,device_record.status,device_record.emp_id  from device_record  inner join device on device.id=device_record.device_id  inner join user on user.id=device_record.emp_id WHERE CONCAT(`request_time`, `assigned_time`, `return_time`,`reject_time`) LIKE '%".$valueToSearch."%' order by request_time DESC LIMIT $start_from, $limit";
